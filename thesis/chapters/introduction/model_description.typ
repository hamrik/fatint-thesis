...
A FATINT modell (melynek neve a #text("Fat Interactions", lang: "en"),
magyarul _kövér interakciók_ rövidítése) egy olyan egyed szimulációs modell,
melynek célja a nyitott evolúció demonstrálása egy stabil, de folyamatosan
változó "ökoszisztéma" fenntartásával. Ezt arra a megfigyelésre alapozza, hogy
való életben az élőlények génjeinek működésére hatást gyakorol a környezetük és
az élőlények egymással való viselkedése.

#definition(caption: "A nyitott evolúció fogalma")[
...
#definition(caption: "A releváns genetikai kifejezések fogalma")[
...
#definition(caption: "A faj fogalma")[
...
- Új gének aktiválásakor a párosodási preferenciák megváltozásának következtében
  a meglévő fajok összeolvadhatnak vagy széteshetnek.
...
#pseudocode-listing(caption: "A FATINT szimuláció egy köre")[
  ...
  + Minden kompatibilis pár a @offspring-count-formula egyenletnek megfelelő
    számú utódot hoz létre, a genotípusuk távolsága, $M_"limit"$, $M_"const"$ és $M_"slope"$ függvényében, lásd @fatint-reproduction-steps.
...
#pseudocode-listing(caption: "Utód genotípusának öröklődése")[
  ...
  + $P_"mutation"$ valószínűséggel a gént véletlenszerűen módosítjuk
    $[-V_"mutation", V_"mutation"]$ közötti mértékkel.
  + Ha a gén egy meghatározott $[V_"min", V_"max"]$ allél tartományon kívül
    esik, az utód életképtelen és eltávolítjuk.
] <fatint-reproduction-steps>
...
melyeket a @energy-params, @reproduction-params, @genetic-params és @init-params részleteznek.

#definition(caption: "Energiával kapcsolatos paraméterek")[
...
/ $E_"intake" in NN$, avagy energia bevitel: Ennyi energiát próbál felvenni a
  környezetből minden egyes egyed a szimulációs kör folyamán. Kevesebb energiát
...
#definition(caption: "Szaporodással kapcsolatos paraméterek")[
...
/ $M_"const" in NN$, avagy minimum gyermekszám: Két kompatibilis egyed párosodása során
...
#definition(caption: "Genetikai paraméterek")[
...
#definition(caption: "Inicializációs paraméterek")[
...
A @model-defaults táblázat felsorolja minden paraméter típusa, tartománya és alapértelmezett értéke.
...
Mivel a FATINT modell véletlenszámokat használ, ezért egyetlen szimuláció
nem elég ahhoz, hogy következtetéseket vonjunk le az állapotában alakulásából.
Ugyanazon kezdő paraméterekkel több szimulációt is futtatni kell úgy, hogy
mindig más véletlenszámokat kap a modell.
Több szimuláció állapotainak átlagolásával a véletlenszámok megbízhatóbb képet
kapunk arról, hogy valóban a paraméterek hatottnak a modell viselkedésére és nem a
véletlenszámok.
Ezt hívjuk *kísérletnek* (@define-experiment).

#definition(caption: "A kísérlet fogalma")[
  *Kísérletnek* nevezzük a szimuláció többszöri futtatását ugyanazon kezdő
  paraméterekkel, de különböző véletlen számokkal.
] <define-experiment>

Egy kísérletből még nem állapítható meg, hogy egy adott paraméter hogyan befolyásolja
a modell viselkedését. Egy paraméter több lehetséges értékével is el kell végezni
egy-egy kísérletet, és azok eredményeit összehasonlítani. Ezt hívjuk *kísérletsornak*
(@define-experiment-sweep).

#definition(caption: "A kísérletsor fogalma")[
  *Kísérletsor:* Kísérletek futtatása az egyik paraméter több lehetséges értékével, a többi
