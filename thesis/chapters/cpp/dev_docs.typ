...
és a @fatint cikkben bemutatott eredmények replikációja. Ehhez nyújt egy rugalmas
ls könnyen bővíthető könyvtárat, valamint egy parancssori eszközt, mellyel könnyen
és gyorsan futtathatók nagy méretű kísérletek és kísérletsorok.

==== Funkcionális követelmények

Az implementációnak a következő funkcionalitással rendelkeznie kell:
...
Az implementációnak a következő tulajdonságokkal kell rendelkeznie:
...
#figure(
  rotate(-90deg, image("/assets/diagrams/cpp_simulator_classes.svg"), reflow: true),
  caption: [A `Simulator` osztály és függőségeinek osztálydiagramja],
) <libfatint-simulator-class-diagram>
#figure(
  rotate(-90deg, image("/assets/diagrams/cpp_simulator_params.svg"), reflow: true),
...
#figure(
  image("/assets/diagrams/cpp_experiment_classes.svg"),
...
Az egyes elemek egy adatcsővezetéket (_"#text("data pipeline", lang: "en")"_)
alkotnak, több transzformációs lépéssel. A parancssori program és a könytár
egymás közti interakcióit listázza a @libfatint-dataflow.
...
+ Ha nincs hiba, akkor a program minden egyes `RunParameter` objektumhoz
...
Alapértelmezett paraméterek mellett a idővel egyetlen faj kerekedik felül, lásd
@cpp-species-comp-default.
...
$P_"encounter"$ alacsony értékeknél nem születik elég utód a fennmaradáshoz.
Magasabb érték mellet egy faj elszaporodik, mert semmi nem segíti elő a
génállomány diverzifikációját.
@cpp-species-comp-p-encounter.
...
$P_"mutation"$ magasabb értékeknél a domináns fajból kiválhat egy-egy
kérészéletű faj, de mivel ezen fajok gyakran egykék, nem hoznak utódot
és kihalnak. Hosszú távon továbbra is egyetlen faj dominál. Lásd @cpp-species-comp-p-mutation.
...
$P_"crossing"$ nincs befolyással a fajok alakulására, mert mindkét szülő
definícióból ugyanazon fajhoz tartozik, így diverzifikáció szempontjából
nem lényeges, hogy az utódban mely szülő génjei dominálnak.
Lásd @cpp-species-comp-p-crossing.
...
A $P_"change"$ paraméter már valóban képes új fajokat létrehozni, mert egyszerre
hat minden egyedre, nem csak az utódokra. Valahányszor új gén adódik az egyedek
genotípusához, a párválasztási preferenciák véletlenszerűen elmozdulnak, mely
fajok töredezéséhez vagy egybeolvadásához vezethet. Ezek az események tüskék
formájában láthatók a @cpp-species-comp-p-change grafikonon. A legtöbb új faj
vagy nem életképes (például egyke), vagy a későbbi generációk összeolvasztják
őket más fajokkal. Emiatt a tüske után gyorsan visszazuhan a fajok száma,
azonban nem feltétlenül esik vissza egyre. Minél magasabb $P_"change"$, annál
gyakoribbak a tüskéket kiváltó események.
...
$P_"change" = 0.0005$-el garantálva az új gének hozzáadását és így a diverzifikációt.
Ha $M_"limit"$ alacsony, akkor a preferenciák érzékenyek a genotípusok változására,
így a fajok könnyen töredeznek, nagyobb tüskéket okozva. Viszont az új fajok így
kevésbé lesznek életképesek, nagyobbrészt egykék, így a tüskék után a fajok száma
hamar csökkenni kezd. Magas $M_"limit"$ mellett a fajok jobban ellenállnak a
töredezésnek, mert az egyedek rugalmasabbak a párkeresés során.
Lásd @cpp-species-comp-m-limit.
...
Ha véletlenszerű gének helyett a @stretch-formula egyenletet használjuk,
akkor a tüskék során kialakuló fajok hasonlóak egymáshoz, így könnyen egybeolvadnak.
Az egybeolvadó fajok egyre ellenállóbbak, így a fajok száma lassabban csökken, sőt.
hosszú távon a fajok átlagos száma nő. Lásd @cpp-species-comp-v-stretch.

==== Teljesítmény

A @cpp-species-counter-perf-comp és @cpp-simulation-perf diagramok alapján elmondható, hogy a C++ implementáció
...
== Mesterséges intelligencia használata <cpp-ai>
