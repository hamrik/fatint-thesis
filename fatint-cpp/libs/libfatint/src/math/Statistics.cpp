...
auto
measure(const std::vector<double>& values) -> Measurement
{
  ...
  return { .min = min,
           ...
           .err = standard_deviation / srn };
}

auto
measure(size_t runs, size_t steps, const ExperimentStates& results)
  -> ExperimentResults
{
...
auto
measure(const ExperimentParameters& params, const ExperimentStates& results)
  -> ExperimentResults
{
...
auto
measure(const ExperimentSweepParameters& params,
        const ExperimentSweepStates& results) -> ExperimentSweepResults
{
...
auto
measure(const ExperimentSweepParameters& params,
        const ExperimentStates& results) -> ExperimentSweepResults
{
  ExperimentSweepResults sweep_results;
