...
Random::Random(size_t seed) : pimpl(std::make_unique<impl>())
{
    this->seed(seed);
}
...
auto Random::random(int min, int max) -> int
{
    return std::uniform_int_distribution<>(min, max)(pimpl->rng);
}
auto Random::random(double min, double max_excl) -> double
{
    return std::uniform_real_distribution<>(min, max_excl)(pimpl->rng);
}
auto Random::chance(double p) -> bool
{
    return random(0.0, 1.0) < p;
}
void Random::shuffle(std::vector<size_t> &v)
{
    std::shuffle(v.begin(), v.end(), pimpl->rng);
}
auto Random::random_indices(size_t count) -> std::vector<size_t>
{
