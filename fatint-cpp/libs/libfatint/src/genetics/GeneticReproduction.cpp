...
GeneticReproduction::GeneticReproduction(IMutation &mutation, ICrossover &crossover)
    : mutation(mutation), crossover(crossover)
{
}

auto GeneticReproduction::reproduce(math::Random &random, const model::GeneticProbabilities &probabilities,
                                    ...
                                    const model::Genotype &b) const -> model::Entity
{
    ...
    mutation.mutate(random, probabilities.p_mutation, allele_parameters.v_mutation, child);
