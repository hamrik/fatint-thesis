...
inline auto
entity_energy_change(int age,
                     double energy_taken,
                     double e_discount,
                     double e_consumption) -> double
{
  double energy_gained =
    energy_taken * pow(e_discount, static_cast<double>(age));
  return energy_gained - e_consumption;
}

/// Determines how many entities a pair of compatible entities can create
inline auto
offspring_count(double genetic_distance,
                double m_const,
                double m_limit,
                double m_slope) -> int
{
  ...
  return static_cast<int>(
    floor(m_const - (m_limit - genetic_distance) * m_slope));
}

/// Determines the next gene to be added to the entity given its current genotype
inline auto
stretch_allele(int last_allele, int v_min, int v_max, double v_stretch) -> int
{
