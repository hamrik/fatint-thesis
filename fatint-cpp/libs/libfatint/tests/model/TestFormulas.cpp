...
TEST_CASE("stretch_allele basic wrapping")
{
  // last_allele=5, v_min=0, v_max=10, v_stretch=1.0
  int result = fatint::model::stretch_allele(5, 0, 10, 1.0);
  CHECK(result == 5); // 0 + floor(5 * 1.0) % 11 = 0 + 5 = 5
}

TEST_CASE("stretch_allele with stretching factor")
{
  // Stretch factor of 2.0 doubles the value
  int result = fatint::model::stretch_allele(5, 0, 10, 2.0);
  CHECK(result == 10); // 0 + floor(5 * 2.0) % 11 = 0 + 10 = 10
}

TEST_CASE("stretch_allele wraps around")
{
  // Should wrap around when stretched value exceeds max
  int result = fatint::model::stretch_allele(10, 0, 10, 2.0);
  CHECK(result == 9); // 0 + floor(10 * 2.0) % 11 = 0 + 20 % 11 = 9
}

TEST_CASE("stretch_allele with offset min")
{
  // v_min is not zero
  int result = fatint::model::stretch_allele(5, 10, 20, 1.0);
