...
class DummySpeciesCounter : public fatint::measurement::ISpeciesCounter
{
  ...
  [[nodiscard]] auto count_species(
    const fatint::model::Limits& limits,
...
{
  std::cout << "starting_entity_count,nanoseconds,ending_entity_count\n";
  ...
  for(size_t sz = 1, i = 0; i < SIZES; sz *= 2, i++) {
    ...
    std::cout << sz << "," << dur.count() << "," << output[output.size() - 1].entity_count << "\n";
  }
}

TEST_CASE("SimulatorPerformance - Churn")
{
  std::cout << "e_increase,nanoseconds,entity_end\n";
