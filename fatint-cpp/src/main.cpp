...
auto define_args(cxxopts::Options &options, int argc, char **argv) -> cxxopts::ParseResult
{
    ...
    opt("p,starting_population", "Initial population size", def<unsigned int>("100"));
    opt("g,starting_allele_count", "Initial number of alleles", def<unsigned int>("5"));
    ...
    opt("dg,sweep_starting_allele_count", "Initial number of alleles delta", def<unsigned int>("0"));
...
auto parse_args(cxxopts::ParseResult &result) -> fatint::simulation::ExperimentSweepParameters
{
    try
    {
        ...
        initial_run_parameters.allele_parameters.v_mutation = result["v_mutation"].as<int>();
        initial_run_parameters.allele_parameters.v_stretch = result["v_stretch"].as<double>();
        ...
        delta.reproduction_parameters.starting_population = result["sweep_starting_population"].as<unsigned int>();
        delta.allele_parameters.starting_allele_count = result["sweep_starting_allele_count"].as<unsigned int>();
        ...
        delta.allele_parameters.v_mutation = result["sweep_v_mutation"].as<int>();
...
auto main(int argc, char **argv) -> int
{
    ...
    fatint::simulation::Simulator simulator(similarity, selection, reproduction, validator,
                                            use_vstretch ? (fatint::genetics::IAlleleAdder &)vstretch_adder
                                                         : (fatint::genetics::IAlleleAdder &)random_adder,
                                            use_ds ? (fatint::measurement::ISpeciesCounter &)ds_counter
